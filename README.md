...
### 🛡️ Standalone Enterprise Admin Portal
A completely decoupled, dedicated React application for system administrators to oversee the platform:
*   **Platform Intelligence**: Live analytics on Average Career Readiness, Goal Alignment, and Top Career Domains across all users.
*   **User Management**: Securely promote/demote administrators, delete accounts, and manage access.
*   **Digital Twin Directory**: A comprehensive table viewing every AI-generated twin in the system, tracking their domain and readiness scores.
*   **AI Vector Memory Logs**: Raw visibility into the semantic memory database, tracking exactly what documents the AI processes and when.

---

## 📐 System Architecture

The AI Personal Digital Twin utilizes a **Decoupled 3-Tier Architecture** designed for high modularity and low-latency inference.

```mermaid
graph TD
    subgraph Client_Layer [Face: React Frontends]
        UI[Main User Dashboard]
    ...
    subgraph Data_Layer [Memory: MongoDB]
        Users[(User Identity & Roles)]
        Twins[(Digital Twin JSON)]
        Memories[(AI Vector Memories)]
    end
...
| `/api/resume/upload` | `POST` | PDF Ingestion & Twin Construction | User JWT |
| `/api/job-market/:role`| `GET` | Live Global Market Analytics | User JWT |
| `/api/career/predict` | `GET` | Final Domain Classification | User JWT |
| `/api/chat` | `POST` | Context-Aware AI Mentorship | User JWT |
| `/api/admin/stats` | `GET` | Platform-wide Twin & Readiness analytics | **Admin JWT** |
| `/api/admin/twins` | `GET` | Full Digital Twin Directory | **Admin JWT** |
| `/api/admin/memories` | `GET` | Raw AI Vector Memory logs | **Admin JWT** |
| `/api/admin/users/:id/role` | `PUT` | Escalate/Revoke privileges | **Admin JWT** |
...
### 3. Execution (Running the 3-Tier System)
Open three separate terminal windows to run the Backend, Frontend, and Admin Portal simultaneously:
...
# Terminal 2: Main User Frontend (Runs on Port 5173)
cd frontend
npm install
npm run dev

# Terminal 3: Dedicated Admin Portal (Runs on Port 5174)
cd admin
npm install
npm run dev
```

### 4. Setting up your Administrator Account
To access the Admin Portal (`http://localhost:5174`), you must have an Admin account. We have provided a secure script to automatically provision one for you:

```bash
# Inside the backend/ folder:
node scripts/createDefaultAdmin.js
```
*This will create an account with the email `admin@example.com` and password `AdminPassword123!` which you can use to log into the Admin portal.*
